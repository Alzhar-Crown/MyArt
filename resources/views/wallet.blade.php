    ...
    <div class="w-full h-[100vh] overflow-y-hidden overflow-x-hidden  ">
        ...
        <div class="flex flex-col justify-center items-center -translate-y-[800px]  gap-6 h-[100vh] p-4">
            ...
            <div class=" flex flex-row mx-auto gap-2  w-fit">
                    ...
                    @if ($ordered->count() > 0)
                        @foreach ($ordered as $key => $item)
                            <div
                                ...
                                {{-- headline --}}
                                <div class=" w-[200px] h-[30px] mt-3  ">Produk {{ $item->headline }}</div>

                                {{-- kategori --}}

                                {{-- harga --}}
                                <div class=" w-[20%] h-[30px] mt-3 text-red-600"> - {{ $item->harga }}</div>
                ...
                <div class="border border-green-500 rounded-md shadow-md p-2"
                    style="width: 300px;height:300px; margin: none;">
                    {!! $chart->container() !!}
                </div>

                {!! $chart->script() !!}
                <div class="flex h-[50vh]  overflow-y-scroll  scrollbar-hide p-4 -mt-5 flex-col">
                    <h2 class="text-black font-bold"> Balance entry flow </h2>
                     @if ($topup->count() > 0)
                        @foreach ($topup as $key => $item)
                            <div
                                class="flex transition-all duration-300  flex-row justify-between border text-start  hover:translate-y-2 w-[550px] h-[40%] hover:bg-gray-300 bg-none py-4 px-6  text-black">
                                {{-- no --}}
                                ...
                                {{-- waktu --}}
                                <div class="  h-fit w-[15.6%] text-[10px]   ">{{ $item->external_id }}</div>
                                <div class="  h-fit w-[15.6%] text-[10px]   ">{{ $item->status }}</div>
